...
- STM32F103C8T6 最小系统板
- 网上常见 TB6612 四路电机驱动模块
- 四个 12V 333rpm 520 霍尔编码器减速电机
- PS2 手柄和接收器
...
| 后右轮 | `PA11` | `PB4` | `PB5` | TB6612 #2 B |

四路 TB6612 模块如果有统一 `STBY` / `EN`，接到 `PB2`。`VM` 接 12V 电机电源，逻辑电源按模块要求接 `3V3` 或 `5V`，所有地线共地。`PC13` 保留给最小系统板板载 LED，`PB8/PB9` 保留给 OLED 软件 I2C。
...
电机编码器参数按 `11PPR * 30` 减速比计算，程序四倍频后约为 `1320 count/rev`。333rpm 空载速度换算到 10ms 控制周期约为 `73 count/10ms`，因此默认速度限幅设为 `SPEED_LIMIT_TICKS=35`、`SLOW_SPEED_TICKS=18`，适合初学者先低速调试。
...
- `Drivers/led.*`：PC13 板载 LED 心跳，用于最小系统板基础验证。
- `Drivers/oled.*`：PB8/PB9 软件 I2C OLED，用于显示 PS2 和编码器调试信息。
...
7. 主循环同时让 PC13 LED 以 500ms 周期闪烁，并每 200ms 刷新 OLED 状态。
